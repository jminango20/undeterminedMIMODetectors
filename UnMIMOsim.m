function UnMIMOsim(varargin)
  ...
  if isempty(varargin)
    ...
    %par.detector = {'Prevoting_MMSE_SIC'}; % Definição detectore(s) a serem simulados
    par.detector = {'MLD','Prevoting_ZF','Prevoting_ZF_SIC','Prevoting_MMSE','Prevoting_MMSE_SIC'}; % Definição detectore(s) a serem simulados
  else      
...
%% DETECTOR Prevoting com ZERO FORCING            
            case 'Prevoting_ZF', % Prevoting ZF Detecção - Tradicional            
                tic;
            ...
            case 'Prevoting_ZF_SIC', % ZF-SIC Detecção - Tradicional            
                tic;
...
%% DETECTOR Prevoting com MMMSE                            
            case 'Prevoting_MMSE', % Prevoting ZF Detecção - Tradicional            
                tic;
            ...
            case 'Prevoting_MMSE_SIC', % MMSE-SIC Detecção - Tradicional            
                tic;
